...
namespace MVCWeb.Service
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<ResponseDto?> GetOrders(string userId, int pageSize, int pageNumber)
        {
            return await _baseService.SendAsync(new RequestDto()
            {
                ApiType = SD.ApiType.GET,
                Url = $"{SD.OrderAPIBase}/api/order/Orders/{userId}?pageSize={pageSize}&pageNumber={pageNumber}"
            });
        }
